...
# Repo slug and the branch/ref the release is cut from. In CI, GITHUB_REPOSITORY is
# set by Actions; RELEASE_NOTES_HEAD lets patch releases compare against a vX.Y-release
# branch instead of main.
DEFAULT_REPO = os.environ.get("RELEASE_NOTES_REPO") or os.environ.get("GITHUB_REPOSITORY") or "pollen-robotics/reachy_mini"
HEAD_REF = os.environ.get("RELEASE_NOTES_HEAD", "main")
...
def get_commits_since_tag(repo, tag_name: str) -> list:
    ...
    # Get commits on the release ref since the tag
    comparison = repo.compare(tag_sha, HEAD_REF)
...
def fetch_prs_since_tag(tag_name: str, repo_name: str = DEFAULT_REPO) -> list[int]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--since", required=True, help="Tag to compare against (e.g., v1.8.4)")
