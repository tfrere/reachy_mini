---
name: reachy-mini-release-notes:validate
...
- Follow the main skill's **Bullet style** rules: capitalize the leading summary, drop
  leading numbers and conventional-commit/branch prefixes, and don't restate the section.
...
- If it has the `"highlight"` label, create a new highlight section with:
  ...
  - If the PR introduces a user-facing feature, check for relevant documentation at
    `https://huggingface.co/docs/reachy_mini/` and add a doc link
